...
def init_db():
    conn = conectar_bd()
    cursor = conn.cursor()
    # Criação das tabelas puras do SQLite
    ...
    # INSERÇÃO DIRETA E REAL DO USUÁRIO ADMIN NO BANCO DE DADOS
...
def verificar_credenciais(usuario, senha):
    init_db() # Força a verificação e criação se o banco tiver sumido da pasta /tmp
    conn = conectar_bd()
    cursor = conn.cursor()
